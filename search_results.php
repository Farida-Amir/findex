...
// Optional: Add a message for regular users about upgrade benefits
$user_type = $_SESSION['user_type'] ?? 'user';
$is_shop_or_admin = ($user_type === 'shop' || $user_type === 'admin');
...
<div class="max-w-7xl mx-auto px-4 py-12">
    ...
    <!-- Upgrade Banner for Regular Users (Optional - Encourages shop registration) -->
    <?php if (!$is_shop_or_admin): ?>
    <div class="upgrade-banner rounded-xl p-4 mb-6 flex flex-wrap justify-between items-center">
        <div class="flex items-center gap-3">
            <i class="fas fa-store text-orange-600 text-xl"></i>
            <div>
                <p class="text-sm font-semibold text-gray-800">Upgrade to Shop Account for Premium Features</p>
                <p class="text-xs text-gray-600">Get verified badge, priority support, and advanced analytics</p>
            </div>
        </div>
        <a href="register.php?type=shop" class="bg-orange-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-orange-700 transition mt-2 sm:mt-0">
            Register as Shop →
        </a>
    </div>
    <?php endif; ?>
        ...
        <form method="GET" action="search.php">
            ...
            <div class="flex flex-wrap justify-between items-center gap-3">
                <div class="flex gap-2">
                    <button type="button" onclick="setStatus('')" class="filter-chip <?php echo empty($status_filter) ? 'active' : ''; ?>">All</button>
                    <button type="button" onclick="setStatus('active')" class="filter-chip <?php echo $status_filter === 'active' ? 'active' : ''; ?>">Active</button>
                    <button type="button" onclick="setStatus('resolved')" class="filter-chip <?php echo $status_filter === 'resolved' ? 'active' : ''; ?>">Resolved</button>
                </div>
    ...
    <?php else: ?>
        ...
        <div class="grid md:grid-cols-2 gap-6">
            <div class="glass-card p-6">
                ...
                <div class="flex flex-wrap gap-2">
                    <?php foreach ($popular as $loc): ?>
                        <a href="?location=<?php echo urlencode($loc['location']); ?>" class="px-3 py-1 bg-gray-100 rounded-full text-xs hover:bg-orange-100 hover:text-orange-600 transition">
            ...
            <div class="glass-card p-6">
                ...
                <div class="space-y-2">
                    <a href="?category=lost" class="flex justify-between items-center p-3 bg-amber-50 rounded-xl hover:bg-amber-100 transition text-sm">
                    ...
                    <a href="?category=stolen" class="flex justify-between items-center p-3 bg-red-50 rounded-xl hover:bg-red-100 transition text-sm">
                    ...
                    <a href="?category=found" class="flex justify-between items-center p-3 bg-emerald-50 rounded-xl hover:bg-emerald-100 transition text-sm">

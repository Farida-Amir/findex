...
$stmt = $pdo->prepare("SELECT id, full_name, email FROM users WHERE phone IS NULL OR phone = ''");
$stmt->execute();
...
// Get all users
$stmt = $pdo->prepare("SELECT id, full_name, email FROM users ORDER BY created_at DESC");
$stmt->execute();
    ...
    if (empty($title) || empty($message)) {
        ...
        if ($recipient_type === 'all') {
            ...
            foreach ($users_list as $user) {
                $stmt2 = $pdo->prepare("INSERT INTO notifications (user_id, type, title, message, is_read, created_at) VALUES (?, 'admin', ?, ?, 0, NOW())");
            ...
            foreach ($missing_phone as $user) {
                $stmt2 = $pdo->prepare("INSERT INTO notifications (user_id, type, title, message, link, is_read, created_at) VALUES (?, 'missing_info', ?, ?, 'edit_profile.php', 0, NOW())");
                $stmt2->execute([$user['id'], $title, $message]);
                $sent_count++;
            }
            ...
            foreach ($missing_docs as $shop) {
                $stmt2 = $pdo->prepare("INSERT INTO notifications (user_id, type, title, message, link, is_read, created_at) VALUES (?, 'missing_info', ?, ?, 'shop_verification.php', 0, NOW())");
            ...
            $stmt2 = $pdo->prepare("INSERT INTO notifications (user_id, type, title, message, is_read, created_at) VALUES (?, 'admin', ?, ?, 0, NOW())");
            $stmt2->execute([$specific_user_id, $title, $message]);
...
<div class="admin-gradient text-white py-6">
    <div class="max-w-7xl mx-auto px-6">
        ...
        <p class="text-gray-300 text-sm">Notify users about missing information or platform updates</p>
    </div>
</div>
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden">
        ...
        <div class="p-6">
            ...
            <form method="POST">
                ...
                <button type="submit" name="send_notification" class="bg-orange-600 text-white px-6 py-2 rounded-lg font-semibold hover:bg-orange-700">Send Notification</button>
            </form>
        </div>
    </div>
</div>
